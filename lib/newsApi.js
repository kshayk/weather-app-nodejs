...
var newsApi = function(country, api_key) {
    return new Promise((resolve, p_error) => {
        var url = news_api_endpoint.replace('{country}', country).replace('{api_key}', api_key);
        var everything_url = all_news_api_endpoint.replace('{country}', country).replace('{api_key}', api_key);
